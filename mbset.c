...
#define THIS_MODULE_CLASSIC_NAME	"mbset"
#define THIS_MODULE_MODERN_NAME	"mbset"
...
GMT_LOCAL struct MBSET_CTRL {
	...
	struct mbset_F {	/* -F<format> */
		bool active;
		int format;
	} F;
	struct mbset_I {	/* -I<inputfile> */
		bool active;
		char *file;
	} I;
	struct mbset_L {	/* -L */
	...
	for (opt = options; opt; opt = opt->next) {	/* Process all the options given */
		...
		switch (opt->option) {
			...
			case 'P':	/* Sets the ping averaging */
				...
				for (i = 0; i < strlen(opt->arg); i++) {
					if (opt->arg[i] == ':')
						break;
					else if (opt->arg[i] == '=') {		/* Probably should not do this because it may be an external owned mem */
						opt->arg[i] = ':';
						break;
					}
				}	
...
int GMT_mbset (void *V_API, int mode, void *args) {
	...
	Ctrl = (struct MBSET_CTRL *) New_Ctrl (GMT);	/* Allocate and initialize a new control structure */
	...
	if (Ctrl->E.active) explicit = MB_YES; 
