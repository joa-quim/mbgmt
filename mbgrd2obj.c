...
int GMT_mbgrd2obj (void *V_API, int mode, void *args) {
	...
	if (mode == GMT_MODULE_PURPOSE) return (GMT_mbgrd2obj_usage (API, GMT_MODULE_PURPOSE));	/* Return the purpose of program */
  ...
  int ntriangle = 0;
    ...
    if ((col < G->header->n_columns - 1) && (row < G->header->n_rows - 1)) {
      ...
      if (!(gmt_M_is_dnan (G->data[ij1])
            || gmt_M_is_dnan (G->data[ij2])
            || gmt_M_is_dnan (G->data[ij3]) )) {
        //get surface normal
        a[0] = x[col+1] - x[col];
        a[1] = y[row+1] - y[row+1];
