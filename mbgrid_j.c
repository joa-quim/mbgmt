 ...
 *    $Id: mbgrid.c 2226 2015-02-05 20:38:55Z caress $
 *
...
#define THIS_MODULE_CLASSIC_NAME	"mbgrid_j"
#define THIS_MODULE_MODERN_NAME	"mbgrid_j"
...
EXTERN_MSC int GMT_mbgrid_j(void *API, int mode, void *args);
...
struct MBGRID_CTRL {
	...
	struct mbgrd_tranf {
		...
		int    set_dimensions;
		int    set_spacing;
		int    spacing_priority;
		int    projection_pars_f;
		int    use_NaN;
		int    bathy_in_feet;
		int    gbndset;
		int    check_time;
...
/* interpolation algorithm
	The code is set to use either of two algorithms for 2D thin plate spline
	interpolation. If the USESURFACE preprocessor define is defined then
	the code will use the surface algorithm from GMT. If not, then the zgrid
	algorithm will be used.  - The default is to use zgrid - to
...
int write_ascii(int verbose, char *outfile, float *grid, int nx, int ny, double xmin,
				double xmax, double ymin, double ymax, double dx, double dy, int *error);
int write_arcascii(int verbose, char *outfile, float *grid, int nx, int ny, double xmin,
				   double xmax, double ymin, double ymax, double dx, double dy, double nodata, int *error);
int write_oldgrd(int verbose, char *outfile, float *grid, int nx, int ny, double xmin,
				 double xmax, double ymin, double ymax, double dx, double dy, int *error);
int mbgrid_weight(int verbose, double foot_a, double foot_b, double pcx, double pcy, double dx,
				  double dy, double *px, double *py, double *weight, int *use, int *error);

/* output stream for basic stuff (stdout if verbose <= 1, stderr if verbose > 1) */
FILE	*outfp;

/* program identifiers */
static char rcs_id[] = "$Id: mbgrid.c 2226 2015-02-05 20:38:55Z caress $";
...
/*-----------------------------------------------------------------------------------------*/
void *New_mbgrid_Ctrl (struct GMT_CTRL *GMT) {	/* Allocate and initialize a new control structure */
	struct MBGRID_CTRL *Ctrl;

	Ctrl = gmt_M_memory (GMT, NULL, 1, struct MBGRID_CTRL);

	//Ctrl->M.more = MBCLEAN_FLAG_ONE;

	return (Ctrl);
}

void Free_mbgrid_Ctrl (struct GMT_CTRL *GMT, struct MBGRID_CTRL *Ctrl) {	/* Deallocate control structure */
	if (!Ctrl) return;
...
static int usage (struct GMTAPI_CTRL *API, int level) {
	...
	GMT_Message (API, GMT_TIME_NONE, "\t-Sspike_slope/mode/format -Ttolerance -Wwest/east/south/north \n");
	GMT_Message (API, GMT_TIME_NONE, "\t-Xbeamsleft/beamsright -Ydistanceleft/distanceright -Z\n\t-V -H]\n\n");

	if (level == GMT_SYNOPSIS) return (EXIT_FAILURE);

	GMT_Message (API, GMT_TIME_NONE, "\t<inputfile> is an MB-System datalist referencing the swath data to be gridded.\n");
	...
	for (opt = options; opt; opt = opt->next) {	/* Process all the options given */
		...
		switch (opt->option) {
			...
			case 'D':	/* */
				...
				if (n > 0) {
					Ctrl->D.active = true;
					if (n == 2) Ctrl->transf.set_dimensions = MB_YES;
				}
				else {
			...
			case 'E':	/* */
				if (opt->arg[strlen(opt->arg)-1] == '!') {
					Ctrl->transf.spacing_priority = MB_YES;
				...
				if (n > 0) {
					Ctrl->E.active = true;
					if (n > 1)
						Ctrl->transf.set_spacing = MB_YES;
					if (n < 3)
						strcpy(Ctrl->E.units, "meters");
				}
				else {
			...
			case 'F':	/* */
				n = sscanf(opt->arg, "%d", &Ctrl->F.grid_mode);
				if (n > 0) {
					Ctrl->F.active = true;
				}
				else {
			...
			case 'G':	/* */
				if (opt->arg[0] == '=') {
					...
					n = sscanf(opt->arg, "%d", &Ctrl->G.gridkind);
					if (n > 0) {
						if (Ctrl->G.gridkind == MBGRID_CDFGRD) {
							Ctrl->G.gridkind = MBGRID_GMTGRD;
							Ctrl->G.gridkindstring[0] = '\0';
						}
						else if (Ctrl->G.gridkind > MBGRID_GMTGRD) {
							sprintf(Ctrl->G.gridkindstring, "=%d", (Ctrl->G.gridkind - 100));
							Ctrl->G.gridkind = MBGRID_GMTGRD;
						}
						Ctrl->G.active = true;
					}
			...
			case 'J':	/* */
				...
				if (n > 0) {
					Ctrl->J.active = true;
					Ctrl->transf.projection_pars_f = MB_YES;
				}
				else {
					GMT_Report(API, GMT_MSG_NORMAL, "Syntax error -J option: \n");
					n_errors++;
				}
 				break;
			...
			case 'K':	/* */
				...
				if (n > 0) {
					Ctrl->K.active = true;
					if ((Ctrl->K.grdrasterid = atoi(Ctrl->K.backgroundfile)) <= 0)
			...
			case 'M':	/* */
				n = sscanf(opt->arg, "%d", &(Ctrl->M.more));
				if (n > 0) {
					Ctrl->M.active = true;
				}
				else {
			...
			case 'O':	/* */
				...
				if (n > 0) {
					...
					GMT_Report(API, GMT_MSG_NORMAL, "Syntax error -O option: \n");
					n_errors++;
				}
 				break;
			...
			case 'Q':	/* */
				Ctrl->Q.active = true;
				Ctrl->transf.bathy_in_feet = MB_YES;
 				break;
			...
			case 'U':	/* */
				...
				if (n > 0) {
					Ctrl->U.active = true;
					Ctrl->U.timediff *= 60;
					Ctrl->transf.check_time = MB_YES;
					if (Ctrl->U.timediff < 0.0) {
						Ctrl->U.timediff = fabs(Ctrl->U.timediff);
						Ctrl->transf.first_in_stays = MB_NO;
					}
				}
				else {
...
/*-----------------------------------------------------------------------------------------*/
int GMT_mbgrid_j (void *V_API, int mode, void *args) {
	...
	/* other variables */
	FILE	*dfp, *rfp;
	int	i, j, k, ii, jj, iii, jjj, kkk, ir;
	...
	/* to cheat in the call to mb_write_gmt_grd() */
	int     argc = 1;
	char  **argv = NULL;
	argv = (char **)malloc(1);
	argv[0] = strdup("mbgrid");
	//strcpy(*argv,"mbgrid");
	...
	gbnd[0] = gbnd[1] = gbnd[2] = gbnd[3] = 0.0;
	xdim = 101;
	ydim = 101;
	gxdim = 0;
	gydim = 0;
	...
	clipmode = Ctrl->transf.clipmode;
	setborder = Ctrl->transf.setborder;
	set_dimensions = Ctrl->transf.set_dimensions;
	set_spacing = Ctrl->transf.set_spacing;
	spacing_priority = Ctrl->transf.spacing_priority;
	projection_pars_f = Ctrl->transf.projection_pars_f;
	use_NaN = Ctrl->transf.use_NaN;
	bathy_in_feet = Ctrl->transf.bathy_in_feet;
	gbndset = Ctrl->transf.gbndset;
	check_time = Ctrl->transf.check_time;
	first_in_stays = Ctrl->transf.first_in_stays;
	verbose = GMT->common.V.active;

	if (Ctrl->A.active)
		datatype = Ctrl->A.datatype;
	if (Ctrl->B.active)
		border = Ctrl->B.border;
	if (Ctrl->C.active) {
		clip = Ctrl->C.clip, clipmode = Ctrl->C.clipmode, tension = Ctrl->C.tension;
	}
	if (Ctrl->D.active) {
		xdim = Ctrl->D.xdim, ydim = Ctrl->D.ydim;
	}
	if (Ctrl->E.active) {
		dx_set = Ctrl->E.dx_set, dy_set = Ctrl->E.dy_set;
		strcpy(units, Ctrl->E.units);
	}
	if (Ctrl->F.active)
		grid_mode = Ctrl->F.grid_mode;
	if (Ctrl->G.active) {
		gridkind = Ctrl->G.gridkind;
	...
	if (Ctrl->K.active) {
		...
		grdrasterid = Ctrl->K.grdrasterid;
	}
	if (Ctrl->L.active)
	...
	if (Ctrl->P.active)
		pings = Ctrl->P.pings;
	if (Ctrl->R.active) {
		boundsfactor = Ctrl->R.boundsfactor;
		for (k = 0; k < 4; k++) gbnd[k] = Ctrl->R.gbnd[k];
	}
	if (Ctrl->S.active)
		speedmin = Ctrl->S.speedmin;
	if (Ctrl->T.active)
		tension = Ctrl->T.tension;
	if (Ctrl->U.active)
		timediff = Ctrl->U.timediff;
	if (Ctrl->W.active)
		scale = Ctrl->W.scale;
	if (Ctrl->X.active)
		extend = Ctrl->X.extend;
	...
	GMT_Report (API, GMT_MSG_VERBOSE, "Program %s\nVersion %s\nMB-system Version %s\n",program_name,rcs_id,MB_VERSION);
	...
	if (projection_pars_f == MB_YES)
		...
		if (set_spacing == MB_YES)
			{
			xdim = (int)((gbnd[1] - gbnd[0])/dx_set + 1);
			if (dy_set <= 0.0)
				dy_set = dx_set;
			ydim = (int)((gbnd[3] - gbnd[2])/dy_set + 1);
	...
	/* deal with no projection */
		...
		if (set_spacing == MB_YES
			&& (units[0] == 'M' || units[0] == 'm'))
			{
			xdim = (int)((gbnd[1] - gbnd[0])/(mtodeglon*dx_set) + 1);
			if (dy_set <= 0.0)
				dy_set = mtodeglon * dx_set / mtodeglat;
			ydim = (int)((gbnd[3] - gbnd[2])/(mtodeglat*dy_set) + 1);
			if (spacing_priority == MB_YES)
				{
			...
			&& (units[0] == 'K' || units[0] == 'k'))
			{
			xdim = (int)((gbnd[1] - gbnd[0])*deglontokm/dx_set + 1);
			if (dy_set <= 0.0)
				dy_set = deglattokm * dx_set / deglontokm;
			ydim = (int)((gbnd[3] - gbnd[2])*deglattokm/dy_set + 1);
			if (spacing_priority == MB_YES)
				{
			...
			&& (units[0] == 'F' || units[0] == 'f'))
			{
			xdim = (int)((gbnd[1] - gbnd[0])/(mtodeglon*0.3048*dx_set) + 1);
			if (dy_set <= 0.0)
				dy_set = mtodeglon * dx_set / mtodeglat;
			ydim = (int)((gbnd[3] - gbnd[2])/(mtodeglat*0.3048*dy_set) + 1);
			if (spacing_priority == MB_YES)
				{
	...
	/* output info */
	if (verbose >= GMT_MSG_VERBOSE)
		{
		GMT_Report (API, GMT_MSG_VERBOSE, "\nMBGRID Parameters:\n");
		GMT_Report (API, GMT_MSG_VERBOSE, "List of input files: %s\n",filelist);
		GMT_Report (API, GMT_MSG_VERBOSE, "Output fileroot:     %s\n",fileroot);
		GMT_Report (API, GMT_MSG_VERBOSE, "Input Data Type:     ");
		if (datatype == MBGRID_DATA_BATHYMETRY)
			{
			GMT_Report (API, GMT_MSG_VERBOSE, "Bathymetry\n");
			if (bathy_in_feet == MB_YES)
				GMT_Report (API, GMT_MSG_VERBOSE, "Bathymetry gridded in feet\n");
			}
		else if (datatype == MBGRID_DATA_TOPOGRAPHY)
			{
			GMT_Report (API, GMT_MSG_VERBOSE, "Topography\n");
			if (bathy_in_feet == MB_YES)
				GMT_Report (API, GMT_MSG_VERBOSE, "Topography gridded in feet\n");
			}
		else if (datatype == MBGRID_DATA_AMPLITUDE)
			GMT_Report (API, GMT_MSG_VERBOSE, "Amplitude\n");
		else if (datatype == MBGRID_DATA_SIDESCAN)
			GMT_Report (API, GMT_MSG_VERBOSE, "Sidescan\n");
		else
			GMT_Report (API, GMT_MSG_VERBOSE, "Unknown?\n");
		GMT_Report (API, GMT_MSG_VERBOSE, "Gridding algorithm:  ");
		if (grid_mode == MBGRID_MEDIAN_FILTER)
			GMT_Report (API, GMT_MSG_VERBOSE, "Median Filter\n");
		else if (grid_mode == MBGRID_MINIMUM_FILTER)
			GMT_Report (API, GMT_MSG_VERBOSE, "Minimum Filter\n");
		else if (grid_mode == MBGRID_MAXIMUM_FILTER)
			GMT_Report (API, GMT_MSG_VERBOSE, "Maximum Filter\n");
		else if (grid_mode == MBGRID_WEIGHTED_FOOTPRINT_SLOPE)
			GMT_Report (API, GMT_MSG_VERBOSE, "Footprint-Slope Weighted Mean\n");
		else if (grid_mode == MBGRID_WEIGHTED_FOOTPRINT)
			GMT_Report (API, GMT_MSG_VERBOSE, "Footprint Weighted Mean\n");
		else
			GMT_Report (API, GMT_MSG_VERBOSE, "Gaussian Weighted Mean\n");
		GMT_Report (API, GMT_MSG_VERBOSE, "Grid projection: %s\n", projection_id);
		if (use_projection == MB_YES)
			{
			GMT_Report (API, GMT_MSG_VERBOSE, "Projection ID: %s\n", projection_id);
			}
		GMT_Report (API, GMT_MSG_VERBOSE, "Grid dimensions: %d %d\n",xdim,ydim);
		GMT_Report (API, GMT_MSG_VERBOSE, "Grid bounds:\n");
		if (use_projection == MB_YES)
			{
			GMT_Report (API, GMT_MSG_VERBOSE, "  Eastings:  %9.4f %9.4f\n",gbnd[0],gbnd[1]);
			GMT_Report (API, GMT_MSG_VERBOSE, "  Northings: %9.4f %9.4f\n",gbnd[2],gbnd[3]);
			GMT_Report (API, GMT_MSG_VERBOSE, "  Longitude: %9.4f %9.4f\n",obnd[0],obnd[1]);
			GMT_Report (API, GMT_MSG_VERBOSE, "  Latitude:  %9.4f %9.4f\n",obnd[2],obnd[3]);
			}
		else
			{
			GMT_Report (API, GMT_MSG_VERBOSE, "  Longitude: %9.4f %9.4f\n",gbnd[0],gbnd[1]);
			GMT_Report (API, GMT_MSG_VERBOSE, "  Latitude:  %9.4f %9.4f\n",gbnd[2],gbnd[3]);
				...
				if (boundsfactor > 1.0)
					GMT_Report (API, GMT_MSG_VERBOSE, "  Grid bounds correspond to %f times actual data coverage\n",boundsfactor);
		GMT_Report (API, GMT_MSG_VERBOSE, "Working grid dimensions: %d %d\n",gxdim,gydim);
		if (use_projection == MB_YES)
			{
			GMT_Report (API, GMT_MSG_VERBOSE, "Working Grid bounds:\n");
			GMT_Report (API, GMT_MSG_VERBOSE, "  Eastings:  %9.4f %9.4f\n",wbnd[0],wbnd[1]);
			GMT_Report (API, GMT_MSG_VERBOSE, "  Northings: %9.4f %9.4f\n",wbnd[2],wbnd[3]);
			GMT_Report (API, GMT_MSG_VERBOSE, "Easting interval:  %f %s\n", dx,units);
			GMT_Report (API, GMT_MSG_VERBOSE, "Northing interval: %f %s\n", dy,units);
			if (set_spacing == MB_YES)
				{
				GMT_Report (API, GMT_MSG_VERBOSE, "Specified Easting interval:  %f %s\n", dx_set, units);
				GMT_Report (API, GMT_MSG_VERBOSE, "Specified Northing interval: %f %s\n", dy_set, units);
				}
			}
		else
			{
			...
			GMT_Report (API, GMT_MSG_VERBOSE, "  Longitude: %9.4f %9.4f\n",wbnd[0],wbnd[1]);
			GMT_Report (API, GMT_MSG_VERBOSE, "  Latitude:  %9.4f %9.4f\n",wbnd[2],wbnd[3]);
			GMT_Report (API, GMT_MSG_VERBOSE, "Longitude interval: %f degrees or %f m\n", dx,1000*dx*deglontokm);
			GMT_Report (API, GMT_MSG_VERBOSE, "Latitude interval:  %f degrees or %f m\n",
				dy,1000*dy*deglattokm);
			if (set_spacing == MB_YES)
				{
				GMT_Report (API, GMT_MSG_VERBOSE, "Specified Longitude interval: %f %s\n", dx_set, units);
				GMT_Report (API, GMT_MSG_VERBOSE, "Specified Latitude interval:  %f %s\n", dy_set, units);
				}
			}
		GMT_Report (API, GMT_MSG_VERBOSE, "Input data bounds:\n");
		GMT_Report (API, GMT_MSG_VERBOSE, "  Longitude: %9.4f %9.4f\n",bounds[0],bounds[1]);
		GMT_Report (API, GMT_MSG_VERBOSE, "  Latitude:  %9.4f %9.4f\n",bounds[2],bounds[3]);
		if (grid_mode == MBGRID_WEIGHTED_MEAN)
			GMT_Report (API, GMT_MSG_VERBOSE, "Gaussian filter 1/e length: %f grid intervals\n", scale);
		if (grid_mode == MBGRID_WEIGHTED_FOOTPRINT_SLOPE
						|| grid_mode == MBGRID_WEIGHTED_FOOTPRINT)
			GMT_Report (API, GMT_MSG_VERBOSE, "Footprint 1/e distance: %f times footprint\n", scale);
		if (check_time == MB_YES && first_in_stays == MB_NO)
			GMT_Report (API, GMT_MSG_VERBOSE, "Swath overlap handling:       Last data used\n");
		if (check_time == MB_YES && first_in_stays == MB_YES)
			GMT_Report (API, GMT_MSG_VERBOSE, "Swath overlap handling:       First data used\n");
		if (check_time == MB_YES)
			GMT_Report (API, GMT_MSG_VERBOSE, "Swath overlap time threshold: %f minutes\n", timediff/60.);
		if (clipmode == MBGRID_INTERP_NONE)
			GMT_Report (API, GMT_MSG_VERBOSE, "Spline interpolation not applied\n");
		else if (clipmode == MBGRID_INTERP_GAP)
			{
			GMT_Report (API, GMT_MSG_VERBOSE, "Spline interpolation applied to fill data gaps\n");
			GMT_Report (API, GMT_MSG_VERBOSE, "Spline interpolation clipping dimension: %d\n",clip);
			GMT_Report (API, GMT_MSG_VERBOSE, "Spline tension (range 0.0 to infinity): %f\n",tension);
			}
		else if (clipmode == MBGRID_INTERP_NEAR)
			{
			GMT_Report (API, GMT_MSG_VERBOSE, "Spline interpolation applied near data\n");
			GMT_Report (API, GMT_MSG_VERBOSE, "Spline interpolation clipping dimension: %d\n",clip);
			GMT_Report (API, GMT_MSG_VERBOSE, "Spline tension (range 0.0 to infinity): %f\n",tension);
			}
		else if (clipmode == MBGRID_INTERP_ALL)
			{
			GMT_Report (API, GMT_MSG_VERBOSE, "Spline interpolation applied to fill entire grid\n");
			GMT_Report (API, GMT_MSG_VERBOSE, "Spline tension (range 0.0 to infinity): %f\n",tension);
			}
		if (grdrasterid == 0)
			GMT_Report (API, GMT_MSG_VERBOSE, "Background not applied\n");
		else if (grdrasterid < 0)
			GMT_Report (API, GMT_MSG_VERBOSE, "Background obtained using grd2xyz from GMT grid file: %s\n",backgroundfile);
		else
			GMT_Report (API, GMT_MSG_VERBOSE, "Background obtained using grdraster from dataset: %d\n",grdrasterid);
		if (gridkind == MBGRID_ASCII)
			GMT_Report (API, GMT_MSG_VERBOSE, "Grid format %d:  ascii table\n",gridkind);
		else if (gridkind == MBGRID_CDFGRD)
			GMT_Report (API, GMT_MSG_VERBOSE, "Grid format %d:  GMT version 2 grd (netCDF)\n",gridkind);
		else if (gridkind == MBGRID_OLDGRD)
			GMT_Report (API, GMT_MSG_VERBOSE, "Grid format %d:  GMT version 1 grd (binary)\n",gridkind);
		else if (gridkind == MBGRID_ARCASCII)
			GMT_Report (API, GMT_MSG_VERBOSE, "Grid format %d:  Arc/Info ascii table\n",gridkind);
		else if (gridkind == MBGRID_GMTGRD)
			{
			GMT_Report (API, GMT_MSG_VERBOSE, "Grid format %d:  GMT grid\n",gridkind);
			if (strlen(gridkindstring) > 0)
				GMT_Report (API, GMT_MSG_VERBOSE, "GMT Grid ID:     %s\n",gridkindstring);
			}
		if (use_NaN == MB_YES)
			GMT_Report (API, GMT_MSG_VERBOSE, "NaN values used to flag regions with no data\n");
		else
			GMT_Report (API, GMT_MSG_VERBOSE, "Real value of %f used to flag regions with no data\n", outclipvalue);
		if (more == MB_YES)
			GMT_Report (API, GMT_MSG_VERBOSE, "Data density and sigma grids also created\n");
		GMT_Report (API, GMT_MSG_VERBOSE, "MBIO parameters:\n");
		GMT_Report (API, GMT_MSG_VERBOSE, "  Ping averaging:       %d\n",pings);
		GMT_Report (API, GMT_MSG_VERBOSE, "  Longitude flipping:   %d\n",lonflip);
		GMT_Report (API, GMT_MSG_VERBOSE, "  Speed minimum:      %4.1f km/hr\n",speedmin);
		}
	...
	if (grdrasterid != 0)
		...
		if (grdrasterid > 0)
			...
			GMT_Report (API, GMT_MSG_VERBOSE, "Executing: %s\n", plot_cmd);
		...
		if (use_projection == MB_NO)
			...
			{
			sprintf(plot_cmd, "grd2xyz tmpgrdsample%d.grd -S -bo", pid);
			}
		GMT_Report(API, GMT_MSG_NORMAL, "Executing: %s\n", plot_cmd);
		if ((rfp = popen(plot_cmd,"r")) != NULL)
			{
	...
	*/
	memset(gridsmall, 0, sxdim * sydim * sizeof(double));
	memset(cnt, 0, sxdim * sydim * sizeof(int));
	...
	while ((status = mb_datalist_read2(verbose,datalist,
		...
		if (file_in_bounds == MB_YES)
			...
			/* call mb_read_init() */
			if ((status = mb_read_init(verbose,rfile,rformat,pings,lonflip,bounds,
			                           btime_i,etime_i,speedmin,timegap, &mbio_ptr,&btime_d,&etime_d,
		...
		if (verbose > 0 || file_in_bounds == MB_YES)
			GMT_Report(API, GMT_MSG_VERBOSE, "%d data points processed in %s\n", ndatafile,rfile);
	...
	mb_surface(verbose, ndata, sxdata, sydata, szdata,
		(wbnd[0] - bdata_origin_x), (wbnd[1] - bdata_origin_x),
				(wbnd[2] - bdata_origin_y), (wbnd[3] - bdata_origin_y), sdx, sdy,
		tension, sgrid);
#else
	/* allocate and initialize sgrid */
			...
			while (error <= MB_ERROR_NO_ERROR)
				...
				if ((datatype == MBGRID_DATA_BATHYMETRY
					...
					if (use_projection == MB_YES)
						...
						for (ib=0;ib<beams_bath;ib++)
							if (mb_beam_ok(beamflag[ib]))
								mb_proj_forward(verbose, pjptr, bathlon[ib], bathlat[ib],
						...
						if (mb_beam_ok(beamflag[ib]))
							...
							if (topo_type != MB_TOPOGRAPHY_TYPE_MULTIBEAM)
								...
								if (ix >= 0 && ix < gxdim && iy >= 0 && iy < gydim)
									...
									sigma[kgrid] = sigma[kgrid] + file_weight*topofactor*topofactor *bath[ib]*bath[ib];
									num[kgrid]++;
									cnt[kgrid]++;
									ndata++;
									ndatafile++;
							...
							/* else deal with multibeam data that have beam footprints */
								...
								isx = (int)((bathlon[ib] - wbnd[0] + 0.5 * sdx)/sdx);
								isy = (int)((bathlat[ib] - wbnd[2] + 0.5 * sdy)/sdy);
...
ib, ix, iy, bathlon[ib], bathlat[ib], bath[ib], navlon, navlat);*/
	...
	xx0, yy0, xx1, yy1, xx2, yy2);*/
									...
									/* else for xyz data without footprint */
										...
										norm[kgrid] = norm[kgrid] + file_weight;
										grid[kgrid] = grid[kgrid] + file_weight*topofactor*bath[ib];
										sigma[kgrid] = sigma[kgrid] + file_weight*topofactor*topofactor
												*bath[ib]*bath[ib];
										num[kgrid]++;
										cnt[kgrid]++;
										ndata++;
...
ib, ix, iy, bathlon[ib], bathlat[ib], bath[ib], dx, dy, wbnd[0], wbnd[1]); */
				...
				time_ok = MB_YES;
				  ...
				  /* process if in region of interest */
				  if (grid_mode == MBGRID_WEIGHTED_MEAN && ix >= -xtradim
				&& ix < gxdim + xtradim
				&& iy >= -xtradim
				&& iy < gydim + xtradim
				&& time_ok == MB_YES)
				   ...
				   sigma[kgrid] = sigma[kgrid] + weight*topofactor*topofactor *bath[ib]*bath[ib];
				   num[kgrid]++;
				   if (ii == ix && jj == iy)
					cnt[kgrid]++;
				   }
		...
		if (verbose >= 2)
			GMT_Report(API, GMT_MSG_VERBOSE, "\n");
		if (verbose > 0 || file_in_bounds == MB_YES)
			GMT_Report(API, GMT_MSG_NORMAL, "%d data points processed in %s\n", ndatafile,rfile);
			  ...
			  for (ib=0;ib<beams_bath;ib++)
				...
				if (time_ok == MB_YES && cnt[kgrid] >= num[kgrid])
				  ...
				  if ((data[kgrid] = (double *)
					...
					mb_error(verbose,error,&message);
					GMT_Report(API, GMT_MSG_NORMAL, "\nMBIO Error allocating data arrays:\n%s\n", message);
					GMT_Report(API, GMT_MSG_NORMAL, "The weighted mean algorithm uses much less\n");
					GMT_Report(API, GMT_MSG_NORMAL, "memory than the median filter algorithm.\n");
					GMT_Report(API, GMT_MSG_NORMAL, "You could also try using ping averaging to\n");
					GMT_Report(API, GMT_MSG_NORMAL, "reduce the number of data points to be gridded.\n");
					GMT_Report(API, GMT_MSG_NORMAL, "\nProgram <%s> Terminated\n", program_name);
					mb_memory_clear(verbose, &error);
					Return(error);
					}
...
fprintf(stderr,"%d %f\n",i,sdata[3*i+2]);
	...
	if (grdrasterid != 0 && nbackground > 0)
		...
		mb_surface(verbose, nbackground, bxdata, bydata, bzdata,
			(float)(wbnd[0] - bdata_origin_x), (float)(wbnd[1] - bdata_origin_x),
						(float)(wbnd[2] - bdata_origin_y), (float)(wbnd[3] - bdata_origin_y), dx, dy,
			tension, sgrid);
#else
		cay = (float)tension;
		xmin = (float)(wbnd[0] - 0.5 * dx - bdata_origin_x);
	...
	GMT_Report(API, GMT_MSG_VERBOSE, "Maximum number of data in a bin: %d\n",nmax);
	GMT_Report(API, GMT_MSG_VERBOSE, "Minimum value: %10.2f   Maximum value: %10.2f\n", zmin,zmax);
	GMT_Report(API, GMT_MSG_VERBOSE, "Minimum sigma: %10.5f   Maximum sigma: %10.5f\n", smin,smax);
		...
		double dx, double dy, int *error)
{
	...
	int	i;
	time_t	right_now;
	char	date[32], user[MB_PATH_MAXLINE], *user_ptr, host[256] = {""};
	char	*ctime();
	char	*getenv();
