...
#define THIS_MODULE_CLASSIC_NAME	"mbgrdtiff"
#define THIS_MODULE_MODERN_NAME	"mbgrdtiff"
...
//					"Syntax error: Must specify a map projection with the -J option\n");
	n_errors += gmt_M_check_condition (GMT, !Ctrl->C.file && !Ctrl->I.do_rgb, 
...
int GMT_mbgrdtiff (void *V_API, int mode, void *args) {
	...
	if (use_intensity_grid) {	/* Illumination grid wanted */
		...
		if ((Intens_orig = GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_CONTAINER_ONLY, NULL, Ctrl->Intensity.file, NULL)) == NULL) {	/* Get header only */
			Return (API->error);
		}
	}
	...
	if (!Ctrl->D.active) {
		for (k = 0; k < n_grids; k++) {
			if ((Grid_orig[k] = GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_CONTAINER_ONLY, NULL, Ctrl->I.file[k], NULL)) == NULL) {	/* Get header only */
				Return (API->error);
			}
		}
...
//	}
	...
	for (k = 0; k < n_grids; k++) {
		if (GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_DATA_ONLY, wesn, Ctrl->I.file[k], Grid_orig[k]) == NULL) {	/* Get grid data */
			Return (API->error);
		}
	}
	...
	if (use_intensity_grid) {	/* Illumination wanted */
		...
		if (GMT_Read_Data (API, GMT_IS_GRID, GMT_IS_FILE, GMT_IS_SURFACE, GMT_DATA_ONLY, wesn, Ctrl->Intensity.file, Intens_orig) == NULL) {
	...
	if (need_to_project) {	/* Need to resample the grd file */
		...
		for (k = 0; k < n_grids; k++) {
			...
			gmt_M_err_fail (GMT, gmt_project_init (GMT, Grid_proj[k]->header, inc, nx_proj, ny_proj, Ctrl->E.dpi, grid_registration), Ctrl->I.file[k]);
			gmt_set_grddim (GMT, Grid_proj[k]->header);
			if (GMT_Create_Data (API, GMT_IS_GRID, GMT_IS_SURFACE, GMT_DATA_ONLY, NULL, NULL, NULL, 0, 0, Grid_proj[k]) == NULL) Return (API->error);
		...
		if (use_intensity_grid) {
			...
			gmt_M_err_fail (GMT, gmt_project_init (GMT, Intens_proj->header, inc, nx_proj, ny_proj, Ctrl->E.dpi, grid_registration), Ctrl->Intensity.file);
			gmt_set_grddim (GMT, Intens_proj->header);
			if (GMT_Create_Data (API, GMT_IS_GRID, GMT_IS_SURFACE, GMT_DATA_ONLY, NULL, NULL, NULL, 0, 0, Intens_proj) == NULL) Return (API->error);
	...
	for (i=0;i<NUMBER_TAGS;i++)
	  {
		...
		mb_put_binary_short(MB_NO, value_short, &tiff_header[keyindex]);
		keyindex += 2;
		value_short = (short)strlen(tiff_comment);
		mb_put_binary_short(MB_NO, value_short, &tiff_header[keyindex]);
		keyindex += 2;
		value_short = 0;
		mb_put_binary_short(MB_NO, value_short, &tiff_header[keyindex]);
		...
		value_int = (short)strlen(tiff_comment);
		mb_put_binary_int(MB_NO, value_int, &tiff_header[index]);
		index += 4;
		value_int = tiff_offset[i];
	        mb_put_binary_int(MB_NO, value_int, &tiff_header[index]);
...
header_work->n_columns,header_work->n_rows,header_work->registration,
...
//	}
	...
	/* Free bitimage arrays. gmt_M_free will not complain if they have not been used (NULL) */
        if (P && P->is_bw) gmt_M_free (GMT, bitimage_1);
        if (bitimage_8) gmt_M_free (GMT, bitimage_8);
	if (bitimage_24) gmt_M_free (GMT, bitimage_24);
