...
#define THIS_MODULE_CLASSIC_NAME	"mbprocess"
#define THIS_MODULE_MODERN_NAME	"mbprocess"
#define THIS_MODULE_LIB		"mbgmt"
#define THIS_MODULE_PURPOSE	"Tool for processing swath sonar bathymetry data"
...
/* function prototypes */
GMT_LOCAL int check_ss_for_bath(int verbose, int nbath, char *beamflag, double *bath, double *bathacrosstrack, int nss, double *ss,
                      double *ssacrosstrack, int *error);
GMT_LOCAL int get_corrtable(int verbose, double time_d, int ncorrtable, int ncorrangle, struct mbprocess_sscorr_struct *corrtable,
                  struct mbprocess_sscorr_struct *corrtableuse, int *error);
GMT_LOCAL int get_anglecorr(int verbose, int nangle, double *angles, double *corrs, double angle, double *corr, int *error);

static char rcs_id[] = "$Id: mbprocess.c 2319 2017-10-17 01:34:44Z caress $";

/* Control structure for mbgetdata */
static struct MBPROCESS_CTRL {
	int     read_datalist;
	void	*datalist;

	struct mbprocess_F {	/* -F<format> */
		bool active;
		int format;
	} F;
	struct mbprocess_I {	/* -I<inputfile> */
		bool active;
		char *file;
	} I;
	struct mbprocess_N {	/* -N */
		bool active;
	} N;
	struct mbprocess_O {	/*  */
		bool active;
...
static void *New_Ctrl (struct GMT_CTRL *GMT) {	/* Allocate and initialize a new control structure */
	struct  MBPROCESS_CTRL *Ctrl;

	Ctrl = gmt_M_memory (GMT, NULL, 1, struct MBPROCESS_CTRL);
...
static void Free_Ctrl (struct GMT_CTRL *GMT, struct MBPROCESS_CTRL *Ctrl) {	/* Deallocate control structure */
	if (!Ctrl) return;
...
static int usage (struct GMTAPI_CTRL *API, int level) {
	...
	GMT_Message(API, GMT_TIME_NONE, "usage: mbprocess -Iinfile [-F<format> -P -N -O<file> -S -T -V]\n");
	GMT_Message(API, GMT_TIME_NONE, "MB-System Version %s\n", MB_VERSION);

	if (level == GMT_SYNOPSIS) return (EXIT_FAILURE);

	GMT_Message(API, GMT_TIME_NONE, "This program performs a number of functions, including:\n\
  - merging navigation\n\
  - recalculating bathymetry from travel time and angle data\n\
    by raytracing through a layered water sound velocity model.\n\
  - applying changes to ship draft, roll bias and pitch bias\n\
...
static int parse (struct GMT_CTRL *GMT, struct MBPROCESS_CTRL *Ctrl, struct GMT_OPTION *options) {
...
/*--------------------------------------------------------------------*/
int GMT_mbprocess (void *V_API, int mode, void *args) {
	...
	Ctrl = (struct MBPROCESS_CTRL *) New_Ctrl (GMT);	/* Allocate and initialize a new control structure */
	if ((error = parse (GMT, Ctrl, options))) Return (error);

	/*---------------------------- This is the mbprocess main code ----------------------------*/
	...
	if (Ctrl->N.active) strip_comments = MB_YES;
	if (Ctrl->P.active) checkuptodate = MB_NO;
	if (Ctrl->S.active) printfilestatus = MB_YES;
	if (Ctrl->T.active) testonly = MB_YES;
	...
	if (mbp_ifile_specified == MB_NO) {
		...
		GMT_Report (API, GMT_MSG_NORMAL, "The input file may be specified with the -I option.\n");
		GMT_Report (API, GMT_MSG_NORMAL, "The default input file is \"datalist.mb-1\".\n");
	...
	/* print starting info statements */
		...
		GMT_Report (API, GMT_MSG_NORMAL, "\nProgram <%s>\n", program_name);
		GMT_Report (API, GMT_MSG_NORMAL, "Version %s\n", rcs_id);
		...
		if (checkuptodate == MB_YES)
			GMT_Report (API, GMT_MSG_NORMAL, "  Files processed only if out of date.\n");
		else
			GMT_Report (API, GMT_MSG_NORMAL, "  All files processed.\n");
		if (strip_comments == MB_NO)
			GMT_Report (API, GMT_MSG_NORMAL, "  Comments embedded in output.\n\n");
		else
			GMT_Report (API, GMT_MSG_NORMAL, "  Comments stripped from output.\n\n");
	}
	...
	while (read_data == MB_YES) {
		...
		if (Ctrl->read_datalist == MB_NO) {
			if (Ctrl->O.active)
				strcpy(process.mbp_ofile, Ctrl->O.file);
		...
		if (status == MB_SUCCESS && !Ctrl->O.active && process.mbp_ofile[0] != '/' && process.mbp_ofile[1] != ':' &&
		...
		if ((fstat = stat(mbp_ifile, &file_status)) == 0 && (file_status.st_mode & S_IFMT) != S_IFDIR) {
			ifilemodtime = (int)file_status.st_mtime;
		}
		...
		if ((fstat = stat(mbp_pfile, &file_status)) == 0 && (file_status.st_mode & S_IFMT) != S_IFDIR) {
			pfilemodtime = (int)file_status.st_mtime;
		}
		...
		if (status == MB_FAILURE) {
			...
			if (verbose > 0 || testonly == MB_YES)
				GMT_Report (API, GMT_MSG_NORMAL, "Data skipped - processing unknown: %s\n", mbp_ifile);
		}
		...
		/* skip if input file can't be read */
			...
			proceedprocess = MB_NO;
			if (verbose > 0 || testonly == MB_YES)
				GMT_Report (API, GMT_MSG_NORMAL, "Data skipped - input file cannot be read: %s\n", mbp_ifile);
		}
		...
		/* skip if parameter file can't be read */
			...
			proceedprocess = MB_NO;
			if (verbose > 0 || testonly == MB_YES)
				GMT_Report (API, GMT_MSG_NORMAL, "Data skipped - parameter file cannot be read: %s\n", mbp_pfile);
		}
		/* check for up to date */
			...
			if ((fstat = stat(process.mbp_ofile, &file_status)) == 0 && (file_status.st_mode & S_IFMT) != S_IFDIR)
				ofilemodtime = (int)file_status.st_mtime;
			...
			if (process.mbp_nav_mode != MBP_NAV_OFF && (fstat = stat(process.mbp_navfile, &file_status)) == 0 &&
				...
				navfilemodtime = (int)file_status.st_mtime;
			...
			if (process.mbp_navadj_mode != MBP_NAVADJ_OFF && (fstat = stat(process.mbp_navadjfile, &file_status)) == 0 &&
			    (file_status.st_mode & S_IFMT) != S_IFDIR)
				navadjfilemodtime = (int)file_status.st_mtime;
			...
			if (process.mbp_attitude_mode != MBP_ATTITUDE_OFF && (fstat = stat(process.mbp_attitudefile, &file_status)) == 0 &&
			    (file_status.st_mode & S_IFMT) != S_IFDIR)
				attitudefilemodtime = (int)file_status.st_mtime;
			...
			if (process.mbp_sonardepth_mode != MBP_SONARDEPTH_OFF &&
				...
				sonardepthfilemodtime = (int)file_status.st_mtime;
			...
			GMT_Report (API, GMT_MSG_NORMAL, "%s - %s - %s: \n\tInput:  %s\n\tOutput: %s\n", string1, string2, string3, process.mbp_ifile,
			        process.mbp_ofile);
			if (locked == MB_YES)
				GMT_Report (API, GMT_MSG_NORMAL, "\tLocked by program <%s> run by <%s> on <%s> at <%s>\n", lock_program, lock_user, lock_cpu,
				        lock_date);
			if (testonly == MB_YES || verbose > 0 || printfilestatus == MB_YES) {
				if (outofdate == MB_YES)
					GMT_Report (API, GMT_MSG_NORMAL, "\tFile Status: out of date\n");
				else
					GMT_Report (API, GMT_MSG_NORMAL, "\tFile Status: up to date\n");
				GMT_Report (API, GMT_MSG_NORMAL, "\t\tModification times and ages relative to the output file in seconds:\n");
				mb_get_date_string(verbose, (double)ifilemodtime, dummy);
				GMT_Report (API, GMT_MSG_NORMAL, "\t\t\tInput file:                 %s %12d <%s>\n", dummy, ofilemodtime - ifilemodtime,
				        mbp_ifile);
				if (pfilemodtime > 0) {
					mb_get_date_string(verbose, (double)pfilemodtime, dummy);
					GMT_Report (API, GMT_MSG_NORMAL, "\t\t\tParameter file:             %s %12d <%s>\n", dummy, ofilemodtime - pfilemodtime,
					        mbp_pfile);
				}
				else {
					GMT_Report (API, GMT_MSG_NORMAL, "\t\t\tParameter file:             None\n");
				}
				if (navfilemodtime > 0) {
					mb_get_date_string(verbose, (double)navfilemodtime, dummy);
					GMT_Report (API, GMT_MSG_NORMAL, "\t\t\tNavigation file:            %s %12d <%s>\n", dummy, ofilemodtime - navfilemodtime,
					        process.mbp_navfile);
				}
				else {
					GMT_Report (API, GMT_MSG_NORMAL, "\t\t\tNavigation file:            None\n");
				}
				if (navadjfilemodtime > 0) {
					mb_get_date_string(verbose, (double)navadjfilemodtime, dummy);
					GMT_Report (API, GMT_MSG_NORMAL, "\t\t\tNavigation adjustment file: %s %12d <%s>\n", dummy, ofilemodtime - navadjfilemodtime,
					        process.mbp_navadjfile);
				}
				else {
					GMT_Report (API, GMT_MSG_NORMAL, "\t\t\tNavigation adjustment file: None\n");
				}
				if (attitudefilemodtime > 0) {
					mb_get_date_string(verbose, (double)attitudefilemodtime, dummy);
					GMT_Report (API, GMT_MSG_NORMAL, "\t\t\tSonar depth file:           %s %12d <%s>\n", dummy, ofilemodtime - attitudefilemodtime,
					        process.mbp_attitudefile);
				}
				else {
					GMT_Report (API, GMT_MSG_NORMAL, "\t\t\tSonar depth file:           None\n");
				}
				if (sonardepthfilemodtime > 0) {
					mb_get_date_string(verbose, (double)sonardepthfilemodtime, dummy);
					GMT_Report (API, GMT_MSG_NORMAL, "\t\t\tAttitude file:              %s %12d <%s>\n", dummy,
					        ofilemodtime - sonardepthfilemodtime, process.mbp_sonardepthfile);
				}
				else {
					GMT_Report (API, GMT_MSG_NORMAL, "\t\t\tAttitude file:              None\n");
				}
				if (esfmodtime > 0) {
					mb_get_date_string(verbose, (double)esfmodtime, dummy);
					GMT_Report (API, GMT_MSG_NORMAL, "\t\t\tEdit save file:             %s %12d <%s>\n", dummy, ofilemodtime - esfmodtime,
					        process.mbp_editfile);
				}
				else {
					GMT_Report (API, GMT_MSG_NORMAL, "\t\t\tEdit save file:             None\n");
				}
				if (svpmodtime > 0) {
					mb_get_date_string(verbose, (double)svpmodtime, dummy);
					GMT_Report (API, GMT_MSG_NORMAL, "\t\t\tSVP file:                   %s %12d <%s>\n", dummy, ofilemodtime - svpmodtime,
					        process.mbp_svpfile);
				}
				else {
					GMT_Report (API, GMT_MSG_NORMAL, "\t\t\tSVP file:                   None\n");
				}
				if (ofilemodtime > 0) {
					mb_get_date_string(verbose, (double)ofilemodtime, dummy);
					GMT_Report (API, GMT_MSG_NORMAL, "\t\t\tOutput file:                %s              <%s>\n", dummy, process.mbp_ofile);
				}
				else {
					GMT_Report (API, GMT_MSG_NORMAL, "\t\t\tOutput file:                None\n");
				}
			}
		...
		if (proceedprocess == MB_YES) {
			...
			if (process.mbp_nav_mode == MBP_NAV_ON &&
				...
				GMT_Report (API, GMT_MSG_NORMAL, "\theading, speed, draft, roll, pitch and heave values.\n");
				if (process.mbp_nav_heading == MBP_NAV_ON) {
					GMT_Report (API, GMT_MSG_NORMAL, "Merging of heading data disabled.\n");
					process.mbp_nav_heading = MBP_NAV_OFF;
				}
				if (process.mbp_nav_speed == MBP_NAV_ON) {
					GMT_Report (API, GMT_MSG_NORMAL, "Merging of speed data disabled.\n");
					process.mbp_nav_speed = MBP_NAV_OFF;
				}
				if (process.mbp_nav_draft == MBP_NAV_ON) {
					GMT_Report (API, GMT_MSG_NORMAL, "Merging of draft data disabled.\n");
					process.mbp_nav_draft = MBP_NAV_OFF;
				}
				if (process.mbp_nav_attitude == MBP_NAV_ON) {
					GMT_Report (API, GMT_MSG_NORMAL, "Merging of roll, pitch, and heave data disabled.\n");
			...
			if (process.mbp_bathrecalc_mode == MBP_BATHRECALC_RAYTRACE) {
				...
				if (traveltime != MB_YES) {
					...
					GMT_Report (API, GMT_MSG_NORMAL, "\tTravel times and angles estimated assuming\n");
					GMT_Report (API, GMT_MSG_NORMAL, "\t1500 m/s water sound speed.\n");
				}
			}
			...
			if (process.mbp_ssrecalc_mode == MBP_SSRECALC_ON && process.mbp_format != MBF_EM300MBA &&
				...
				GMT_Report (API, GMT_MSG_NORMAL, "Format %d is specified. Sidescan recalculation disabled\n", process.mbp_format);
			...
			if (verbose == 1) {
				...
				if (strip_comments == MB_YES)
					GMT_Report (API, GMT_MSG_NORMAL, "  Comments in output:            OFF\n");
				else
					GMT_Report (API, GMT_MSG_NORMAL, "  Comments in output:            ON\n");
				...
				if (process.mbp_nav_shift == MBP_NAV_ON) {
					...
					GMT_Report (API, GMT_MSG_NORMAL, "  Navigation shift longitude:%f\n", process.mbp_nav_shiftlon);
					GMT_Report (API, GMT_MSG_NORMAL, "  Navigation shift latitude: %f\n", process.mbp_nav_shiftlat);
				}
				else
				...
				if (process.mbp_navadj_mode >= MBP_NAVADJ_LL) {
					...
					if (process.mbp_navadj_mode == MBP_NAVADJ_LL)
						GMT_Report (API, GMT_MSG_NORMAL, "  Adjusted navigation applied to: lon lat only\n");
					else if (process.mbp_navadj_mode == MBP_NAVADJ_LLZ)
						GMT_Report (API, GMT_MSG_NORMAL, "  Adjusted navigation applied to: lon lat depth_offset\n");
				...
				if (process.mbp_sonardepth_mode == MBP_SONARDEPTH_ON) {
					GMT_Report (API, GMT_MSG_NORMAL, "  Sonardepth merged from sonardepth file.\n");
					GMT_Report (API, GMT_MSG_NORMAL, "  Sonardepth file:                 %s\n", process.mbp_sonardepthfile);
					GMT_Report (API, GMT_MSG_NORMAL, "  Sonardepth format:               %d\n", process.mbp_sonardepth_format);
				}
				else
					GMT_Report (API, GMT_MSG_NORMAL, "  Sonardepth not merged from sonardepth file.\n");
				...
				for (i = 0; i < process.mbp_cut_num; i++) {
					if (process.mbp_cut_kind[i] == MBP_CUT_DATA_BATH)
						GMT_Report (API, GMT_MSG_NORMAL, "  Cut[%d]: bathymetry", i);
					else if (process.mbp_cut_kind[i] == MBP_CUT_DATA_AMP)
						GMT_Report (API, GMT_MSG_NORMAL, "  Cut[%d]: amplitude ", i);
					else if (process.mbp_cut_kind[i] == MBP_CUT_DATA_SS)
						GMT_Report (API, GMT_MSG_NORMAL, "  Cut[%d]: sidescan  ", i);
					if (process.mbp_cut_mode[i] == MBP_CUT_MODE_NUMBER)
						...
						GMT_Report (API, GMT_MSG_NORMAL, "  speed    ");
				...
				if (process.mbp_bathrecalc_mode == MBP_BATHRECALC_OFF)
					...
					GMT_Report (API, GMT_MSG_NORMAL, "  Bathymetry recalculated by sonar depth shift.\n");
				...
				if (process.mbp_static_mode == MBP_STATIC_BEAM_ON) {
					GMT_Report (API, GMT_MSG_NORMAL, "  Static beam corrections applied to bathymetry.\n");
					GMT_Report (API, GMT_MSG_NORMAL, "  Static file:                   %s m\n", process.mbp_staticfile);
				}
				else if (process.mbp_static_mode == MBP_STATIC_ANGLE_ON) {
					GMT_Report (API, GMT_MSG_NORMAL, "  Static angle corrections applied to bathymetry.\n");
					GMT_Report (API, GMT_MSG_NORMAL, "  Static file:                   %s m\n", process.mbp_staticfile);
				}
				else
					GMT_Report (API, GMT_MSG_NORMAL, "  Static beam corrections off.\n");
				...
				if (process.mbp_corrected == MB_YES)
					...
					GMT_Report (API, GMT_MSG_NORMAL, "  Output bathymetry reference:   UNCORRECTED\n");
				...
				if (process.mbp_ampcorr_mode == MBP_AMPCORR_ON) {
					...
					if (process.mbp_ampcorr_slope == MBP_AMPCORR_IGNORESLOPE || process.mbp_ampcorr_slope == MBP_AMPCORR_USESLOPE)
						GMT_Report (API, GMT_MSG_NORMAL, "  Amplitude correction uses swath bathymetry in file\n");
					else {
						GMT_Report (API, GMT_MSG_NORMAL, "  Amplitude correction uses topography grid\n");
				...
				if (process.mbp_sscorr_mode == MBP_SSCORR_ON) {
					...
					if (process.mbp_sscorr_slope == MBP_SSCORR_IGNORESLOPE || process.mbp_sscorr_slope == MBP_SSCORR_USESLOPE)
						GMT_Report (API, GMT_MSG_NORMAL, "  Sidescan correction uses swath bathymetry in file\n");
					else {
						GMT_Report (API, GMT_MSG_NORMAL, "  Sidescan correction uses topography grid\n");
						GMT_Report (API, GMT_MSG_NORMAL, "  Topography grid file:      %s m\n", process.mbp_ampsscorr_topofile);
					}
							...
							if (nsvp == 0) {
								if (depth[0] < 0.0) {
									...
									fprintf(stderr,
									        ...
									        nsvp, depth[0], nsvp, velocity[0]);
									GMT_Report (API, GMT_MSG_NORMAL, "\t\tdepth[%d]: %f  velocity[%d]: %f did not have zero depth\n", nsvp,
			...
			if (process.mbp_nav_mode == MBP_NAV_ON) {
				...
				while ((result = fgets(buffer, nchar, tfp)) == buffer) {
					...
					/* deal with nav in form: yr mon day hour min sec lon lat */
						...
						time_i[5] = (int)sec;
						time_i[6] = (int)(1000000 * (sec - time_i[5]));
					...
					/* deal with nav in form: yr jday hour min sec lon lat */
						...
						time_j[3] = (int)sec;
						time_j[4] = (int)(1000000 * (sec - time_j[3]));
					...
					/* deal with nav in L-DEO processed nav format */
						...
						strncpy(dummy, "\0", 128);
						ioff += 3;
						time_j[2] = (int)(atoi(strncpy(dummy, buffer + ioff, 2)) + 60 * hr);
						strncpy(dummy, "\0", 128);
						ioff += 3;
					...
					/* deal with nav in form: yr mon day hour min sec time_d lon lat heading speed draft*/
						...
						if (nav_ok == MB_YES) {
							...
							if (process.mbp_nav_speed == MBP_NAV_ON && nget < 11) {
								GMT_Report (API, GMT_MSG_NORMAL, "Speed data missing from nav file.\nMerging of speed data disabled.\n");
								process.mbp_nav_speed = MBP_NAV_OFF;
							}
							if (process.mbp_nav_draft == MBP_NAV_ON && nget < 12) {
								GMT_Report (API, GMT_MSG_NORMAL, "Draft data missing from nav file.\nMerging of draft data disabled.\n");
								process.mbp_nav_draft = MBP_NAV_OFF;
							}
							if (process.mbp_nav_attitude == MBP_NAV_ON && nget < 15) {
								GMT_Report (API, GMT_MSG_NORMAL, "Roll, pitch, and heave data missing from nav file.\nMerging of roll, pitch, and "
								                "heave data disabled.\n");
								process.mbp_nav_attitude = MBP_NAV_OFF;
							}
				...
				if (verbose >= 1) {
					...
					GMT_Report (API, GMT_MSG_NORMAL, "Nav start time: %4.4d %2.2d %2.2d %2.2d:%2.2d:%2.2d.%6.6d\n", stime_i[0], stime_i[1],
					        stime_i[2], stime_i[3], stime_i[4], stime_i[5], stime_i[6]);
					GMT_Report (API, GMT_MSG_NORMAL, "Nav end time:   %4.4d %2.2d %2.2d %2.2d:%2.2d:%2.2d.%6.6d\n", ftime_i[0], ftime_i[1],
			...
			if (process.mbp_navadj_mode >= MBP_NAVADJ_LL) {
				...
				if (verbose >= 1) {
					...
					GMT_Report (API, GMT_MSG_NORMAL, "Adjusted nav start time: %4.4d %2.2d %2.2d %2.2d:%2.2d:%2.2d.%6.6d\n", stime_i[0],
					        stime_i[1], stime_i[2], stime_i[3], stime_i[4], stime_i[5], stime_i[6]);
					GMT_Report (API, GMT_MSG_NORMAL, "Adjusted nav end time:   %4.4d %2.2d %2.2d %2.2d:%2.2d:%2.2d.%6.6d\n", ftime_i[0],
			...
			if (process.mbp_attitude_mode == MBP_ATTITUDE_ON) {
				...
				if (verbose >= 1) {
					...
					GMT_Report (API, GMT_MSG_NORMAL, "Attitude start time: %4.4d %2.2d %2.2d %2.2d:%2.2d:%2.2d.%6.6d\n", stime_i[0], stime_i[1],
					        stime_i[2], stime_i[3], stime_i[4], stime_i[5], stime_i[6]);
					GMT_Report (API, GMT_MSG_NORMAL, "Attitude end time:   %4.4d %2.2d %2.2d %2.2d:%2.2d:%2.2d.%6.6d\n", ftime_i[0], ftime_i[1],
					        ftime_i[2], ftime_i[3], ftime_i[4], ftime_i[5], ftime_i[6]);
				}
			}
			...
			if (process.mbp_sonardepth_mode == MBP_SONARDEPTH_ON) {
				...
				if (verbose >= 1) {
					...
					GMT_Report (API, GMT_MSG_NORMAL, "Sonardepth start time: %4.4d %2.2d %2.2d %2.2d:%2.2d:%2.2d.%6.6d\n", stime_i[0], stime_i[1],
					        stime_i[2], stime_i[3], stime_i[4], stime_i[5], stime_i[6]);
					GMT_Report (API, GMT_MSG_NORMAL, "Sonardepth end time:   %4.4d %2.2d %2.2d %2.2d:%2.2d:%2.2d.%6.6d\n", ftime_i[0], ftime_i[1],
					        ftime_i[2], ftime_i[3], ftime_i[4], ftime_i[5], ftime_i[6]);
				}
			}
			...
			if (process.mbp_tide_mode == MBP_TIDE_ON) {
				...
				if (verbose >= 1) {
					GMT_Report (API, GMT_MSG_NORMAL, "\n%d tide records read\n", ntide);
					GMT_Report (API, GMT_MSG_NORMAL, "Tide start time: %4.4d %2.2d %2.2d %2.2d:%2.2d:%2.2d.%6.6d\n", stime_i[0], stime_i[1],
					        stime_i[2], stime_i[3], stime_i[4], stime_i[5], stime_i[6]);
					GMT_Report (API, GMT_MSG_NORMAL, "Tide end time:   %4.4d %2.2d %2.2d %2.2d:%2.2d:%2.2d.%6.6d\n", ftime_i[0], ftime_i[1],
					        ftime_i[2], ftime_i[3], ftime_i[4], ftime_i[5], ftime_i[6]);
				}
			}
			...
			if (process.mbp_sscorr_mode == MBP_SSCORR_ON) {
				...
				else {		/* if no sidescan correction file then quit */
					GMT_Report (API, GMT_MSG_NORMAL, "\nUnable to read data from sidescan correction file <%s>\n", process.mbp_sscorrfile);
					GMT_Report (API, GMT_MSG_NORMAL, "\nProgram <%s> Terminated\n", program_name);
					Return(MB_ERROR_BAD_DATA);
				}
				...
				while ((result = fgets(buffer, MBP_FILENAMESIZE, tfp)) == buffer) {
					...
					if (strncmp(buffer, "# table:", 8) == 0) {
						...
						if (nget != 3) {
							fprintf(stderr, "\n  Error parsing line in sidescan correction file in program <%s>\n",
							        program_name);
			...
			while (error <= MB_ERROR_NO_ERROR) {
				...
				if (verbose >= 1 && error == MB_ERROR_COMMENT) {
					...
					GMT_Report (API, GMT_MSG_NORMAL, "Time: %d %d %d %d %d %d\n", time_i[0], time_i[1], time_i[2],
					...
					mb_error(verbose, error, &message);
					GMT_Report (API, GMT_MSG_NORMAL, "\nFatal MBIO Error:\n%s\n", message);
					GMT_Report (API, GMT_MSG_NORMAL, "Last Good Time: %d %d %d %d %d %d\n", time_i[0], time_i[1],
				...
				if (error == MB_ERROR_NO_ERROR && kind == MB_DATA_DATA && process.mbp_tide_mode == MBP_TIDE_ON && ntide > 0) {
					/* interpolate tide */
				...
				/* if survey data encountered, get the bathymetry */
				if (error == MB_ERROR_NO_ERROR && (kind == MB_DATA_DATA)) {
					...
					/* if svp specified recalculate bathymetry by raytracing  */
					if (process.mbp_bathrecalc_mode == MBP_BATHRECALC_RAYTRACE) {
						/* loop over the beams */
						for (i = 0; i < nbeams; i++) {
							if (ttimes[i] > 0.0) {
								...
								if (depth_offset_use >= depth[0]) {
									...
									if (verbose > 0) {
										...
										GMT_Report (API, GMT_MSG_NORMAL, "of the SVP - transducers above water?!\n");
										GMT_Report (API, GMT_MSG_NORMAL, "Raytracing performed from top of SVP followed by static shift.\n");
										GMT_Report (API, GMT_MSG_NORMAL, "Sonar depth is sum of heave + draft (or transducer depth).\n");
										GMT_Report (API, GMT_MSG_NORMAL, "Draft from data:       %f\n", draft);
										GMT_Report (API, GMT_MSG_NORMAL, "Heave from data:       %f\n", bheave[i]);
										GMT_Report (API, GMT_MSG_NORMAL, "Heave from lever calc: %f\n", lever_heave);
										GMT_Report (API, GMT_MSG_NORMAL, "User specified draft:  %f\n", process.mbp_draft);
										GMT_Report (API, GMT_MSG_NORMAL, "Depth offset used:     %f\n", depth_offset_use);
										GMT_Report (API, GMT_MSG_NORMAL, "Data Record: %d\n", odata);
										GMT_Report (API, GMT_MSG_NORMAL, "Ping time:  %4d %2d %2d %2d:%2d:%2d.%6d\n",
				...
				if (process.mbp_cut_num > 0 && error == MB_ERROR_NO_ERROR && kind == MB_DATA_DATA) {
					for (icut = 0; icut < process.mbp_cut_num; icut++) {
						...
						/* flag data according to beam acrosstrack distance */
						...
						/* flag data according to pixel acrosstrack distance */
						else if (process.mbp_cut_kind[icut] == MBP_CUT_DATA_SS &&
						         process.mbp_cut_mode[icut] == MBP_CUT_MODE_DISTANCE) {
							for (i = 0; i < nss; i++) {
				...
				if ((process.mbp_ampcorr_mode == MBP_AMPCORR_ON && (process.mbp_ampcorr_slope == MBP_AMPCORR_IGNORESLOPE ||
					...
					if (process.mbp_ampcorr_mode == MBP_AMPCORR_ON && !error && kind == MB_DATA_DATA &&
					...
					if (process.mbp_sscorr_mode == MBP_SSCORR_ON && !error && kind == MB_DATA_DATA &&
					...
					if (process.mbp_ampcorr_mode == MBP_AMPCORR_ON && error == MB_ERROR_NO_ERROR && kind == MB_DATA_DATA &&
						...
						for (i = 0; i < namp; i++) {
							if (mb_beam_ok(beamflag[i])) {
								...
								ix = (int)((navlon + r[0] * mtodeglon - grid.xmin + 0.5 * grid.dx) / grid.dx);
								jy = (int)((navlat + r[1] * mtodeglat - grid.ymin + 0.5 * grid.dy) / grid.dy);
								kgrid = ix * grid.ny + jy;
				...
				if (error == MB_ERROR_NO_ERROR || (kind == MB_DATA_COMMENT && strip_comments == MB_NO)) {
					...
					if (status == MB_SUCCESS) {
						...
						GMT_Report (API, GMT_MSG_NORMAL, "Time: %4d %2d %2d %2d:%2d:%2d.%6d\n", time_i[0], time_i[1],
			...
			if (process.mbp_edit_mode == MBP_EDIT_ON) {
				for (i = 0; i < esf.nedit; i++) {
					if (esf.edit[i].use == 1000) {
						neditnull++;
						if (verbose >= 2)
							GMT_Report (API, GMT_MSG_NORMAL, "BEAM FLAG TIED TO NULL BEAM: i:%d edit: %f %d %d   %d\n", i, esf.edit[i].time_d,
									esf.edit[i].beam, esf.edit[i].action, esf.edit[i].use);
					}
					else if (esf.edit[i].use == 100) {
						neditduplicate++;
						if (verbose >= 2)
							GMT_Report (API, GMT_MSG_NORMAL, "DUPLICATE BEAM FLAG:         i:%d edit: %f %d %d   %d\n", i, esf.edit[i].time_d,
									esf.edit[i].beam, esf.edit[i].action, esf.edit[i].use);
					}
					else if (esf.edit[i].use != 1) {
						neditnotused++;
						if (verbose >= 2)
							GMT_Report (API, GMT_MSG_NORMAL, "BEAM FLAG NOT USED:          i:%d edit: %f %d %d   %d\n", i, esf.edit[i].time_d,
									esf.edit[i].beam, esf.edit[i].action, esf.edit[i].use);
					}
					else if (esf.edit[i].use == 1) {
						neditused++;
						if (verbose >= 2)
							GMT_Report (API, GMT_MSG_NORMAL, "BEAM FLAG USED:              i:%d edit: %f %d %d   %d\n", i, esf.edit[i].time_d,
									esf.edit[i].beam, esf.edit[i].action, esf.edit[i].use);
					}
				}
			}
			if (verbose >= 1) {
				GMT_Report (API, GMT_MSG_NORMAL, "          %d flags used\n", neditused);
				GMT_Report (API, GMT_MSG_NORMAL, "          %d flags not used\n", neditnotused);
				GMT_Report (API, GMT_MSG_NORMAL, "          %d flags tied to null beams\n", neditnull);
				GMT_Report (API, GMT_MSG_NORMAL, "          %d duplicate flags\n", neditduplicate);
			}
			...
			if (verbose >= 1) {
				...
				GMT_Report (API, GMT_MSG_NORMAL, "%d input nav records\n", inav);
				GMT_Report (API, GMT_MSG_NORMAL, "%d input comment records\n", icomment);
				GMT_Report (API, GMT_MSG_NORMAL, "%d input other records\n", iother);
				GMT_Report (API, GMT_MSG_NORMAL, "%d output data records\n", odata);
				GMT_Report (API, GMT_MSG_NORMAL, "%d output nav records\n", onav);
				GMT_Report (API, GMT_MSG_NORMAL, "%d output comment records\n", ocomment);
				GMT_Report (API, GMT_MSG_NORMAL, "%d output other records\n", oother);
			}
		...
		if (Ctrl->read_datalist == MB_YES) {
			if ((status = mb_datalist_read(verbose, Ctrl->datalist, mbp_ifile, mbp_dfile, &format, &file_weight, &error)) == MB_SUCCESS)
				read_data = MB_YES;
			else
				read_data = MB_NO;
		}
...
int get_corrtable(int verbose, double time_d, int ncorrtable, int ncorrangle, struct mbprocess_sscorr_struct *corrtable,
	...
	int status = MB_SUCCESS;
	double factor;
	int ifirst, ilast, irecent = 0, inext;
	int i, ii, itable;
